# In this simulation, we want to do experiment to compare different methods in the case you have imbalance cluster size in each cluster
...
d_total = c(4)
p_ind = 80
delta = 0.7 # d= 3's delta candidate seq(0.5,0.8,0.03)   d= 4's delta candidate seq(0.7,1,0.03)
metric = 'ARI'

ratio.candidate <- seq(0.05,0.5,0.05)
...
for (d.index in 1:length(d_total)){
  for (ratio1 in ratio.candidate){
    combined_result <- foreach(i = 1:exp.time) %dopar% {
      ...
      ratio <- c(ratio1, 1-ratio1)
      data = TBM.generator.imbalance(p, r, 10*p[1]^(-delta), ratio)
    ...
    combined_result <- sapply(combined_result, function(err) err)
      ...
      per_result = c(d_total[d.index], ratio1, mean_res,sd_res)
      print(per_result)
...
file_name <- paste("TC_method_compare_imbalance","d", paste(d_total, sep="", collapse="_"),"p", paste(p_ind, sep="", collapse="_"), "r", paste(r_ind, sep="", collapse="_"), "simu", exp.time,'metric',metric, "delta", delta,sep = "_")
print(file_name)
print(final_result)
