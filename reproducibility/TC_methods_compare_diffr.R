# In this simulation, we want to do experiment to compare different methods in the setting that the clustering structure at each mode is different.
source("HOLloyd.R")
library(doParallel)
set.seed(2020)
server = 1
...
p_ind = 80
delta.candidate = seq(0.5,0.9,0.03) # d= 3's delta candidate seq(0.5,0.8,0.03)   d= 4's delta candidate seq(0.7,1,0.03)
metric = 'ARI'
...
file_name <- paste("TC_method_compare_diffr","d", paste(d_total, sep="", collapse="_"),"p", paste(p_ind, sep="", collapse="_"), "r", paste(r, sep="", collapse="_"), "simu", exp.time,'metric',metric, "delta_min", min(delta.candidate), "delta_max", max(delta.candidate),sep = "_")
print(file_name)
print(final_result)
